...
const data = new SlashCommandBuilder()
  .setName("raccoonfact")
  .setDescription("Sends a random raccoon fact!");
...
async function execute(client, interaction, subinteraction, config) {
  async function get_rfact_interaction() {
    ...
    return { embeds: [rfactEmbed], components: [row] };
  }
  if (interaction.isButton()) {
    ...
    } else if (interaction.customId === "newrfact") {
      await interaction.message.edit(await get_rfact_interaction());
  ...
  await interaction.reply(await get_rfact_interaction());
}
